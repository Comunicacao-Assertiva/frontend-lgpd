    ...
    <div>
      ...
      {loading ? (
        <div className="flex items-center justify-center gap-2 py-10 text-sm text-neutral-400">
          <Loader2 size={16} className="animate-spin" /> Carregando do Supabase...
        </div>
      ) : history.length === 0 ? (
        ...
        <div className="space-y-2">
          {history.map(r => (
            <div key={r.supabaseId} onClick={() => onViewResult(r)}
              ...
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0" style={{ background: r.quadrantColor }}>
                {r.quadrantId}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 truncate">{r.organization}</div>
                <div className="text-xs text-neutral-400 mt-0.5">{r.quadrantName} · {formatDate(r.timestamp)}</div>
                <div className="text-xs text-neutral-400 mt-0.5">Gov {r.govPercentage}% · Seg {r.secPercentage}%</div>
              </div>
              <div className="text-xl font-bold flex-shrink-0" style={{ color: r.quadrantColor }}>
              ...
              <button onClick={e => { e.stopPropagation(); onDeleteResult(r.supabaseId); }}
                className="text-neutral-300 hover:text-red-400 transition-colors p-1 flex-shrink-0" title="Remover">
                <Trash2 size={14} />
              </button>
            </div>
          ))}
